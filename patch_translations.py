...
Appends new intro-screen translation keys to translations.py (before the closing `}` of UI_TEXT).
Safe to run repeatedly - skips keys that already exist.
"""
import re, os

FILE = "translations.py"
raw = open(FILE, "rb").read().decode("latin-1")

NEW_KEYS = """
    # ═══════════════════════════════════════════════════════════════════
    # Intro / Language Screen  (revamped)
    # ═══════════════════════════════════════════════════════════════════
    "intro_system_name": {
...
if '"intro_system_name"' in raw:
    print("Already patched – skipping.")
else:
    # Insert before the closing `}` of UI_TEXT dict
    # Find the last occurrence of `}\n\n\ndef t(`
    insert_pos = raw.rfind('\n}\n\ndef t(')
    if insert_pos == -1:
        insert_pos = raw.rfind('\n}\n\n\ndef t(')
    if insert_pos == -1:
        print("Could not find insertion point! Dump tail:")
        print(repr(raw[-300:]))
    else:
        patched = raw[:insert_pos] + NEW_KEYS + raw[insert_pos:]
        with open(FILE, "wb") as f:
            f.write(patched.encode("utf-8"))
        print("Patched successfully – re-encoded as UTF-8.")
